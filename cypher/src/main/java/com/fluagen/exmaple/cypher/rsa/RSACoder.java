...
private static Cipher cipher;
	...
	public static String encrypt(String publicKeystore,String plainText){
		try {			
			...
			byte[] enBytes = cipher.doFinal(plainText.getBytes());	
			System.out.println("加密后：length " + enBytes.length);
			return Base64.getEncoder().encodeToString(enBytes);
		} catch (InvalidKeyException e) {
			e.printStackTrace();
		} catch (IllegalBlockSizeException e) {
	...
	public static String decrypt(PrivateKey privateKey,String enStr){
		try {
			...
			byte[] deBytes = cipher.doFinal(Base64.getDecoder().decode(enStr.getBytes(StandardCharsets.UTF_8)));
	...
	public static void main(String[] args){
		//1. 生成密钥文件
		System.out.println("========生成密钥文件===========");
		String path = "/home/majie/dev/fluagen/example/cypher/src/main/resources/rsa";
		Map<String,String> keys = generateKeyPair(path);
		...
		String cipher = encrypt(path+"/publicKey.keystore", plainText);
		String recover = decrypt(path+"/privateKey.keystore", cipher);
		System.out.println("加密后 : " + cipher);
		System.out.println("解密后 : " + recover);		
	}
}
